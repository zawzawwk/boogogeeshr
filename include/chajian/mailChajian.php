<?php
class mailChajian extends Chajian{
...
function sendMail($email, $html, $tit='')
{
	//邮件信息
	$mail = array(
		'server' =>  'smtp.126.com', 	//SMTP服务器
		'port'   =>  25,              	//SMTP服务器端口
		'user'   =>  'qqqq2900@126.com', //SMTP服务器的用户邮箱
		'pass'   =>  'b111111',  		//密码
		'subject'=>  TITLE, 			//标题
		'type'   =>  'HTML',
		'debug'  =>  false  			//是否显示发送的调试信息
	);
	$smtp 		 = new mailChajian($mail['server'], $mail['port'] ,true, $mail['user'], $mail['pass']);
	$mailbody	 = $html;
	$murl		 = 'http://'.HOST.'/';
	$mailbody	.= '<br>&nbsp;<hr size="1">感谢您的访问，祝您使用愉快！<br>网站：<a href="'.$murl.'" target="_blank">'.TITLE.'</a><br>网址：'.$murl.'<br>时间：'.date('Y-m-d H:i:s').'<br>';
	$subject	 = TITLE;
	if($tit!='')$subject.='_'.$tit.'';
	@$smtp->sendmail($email, $mail['user'], $subject, $mailbody, $mail['type']);//给收件人发
}
